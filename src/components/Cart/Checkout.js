...
const isNotLongEnough = (value) =>
  value.trim().length < 5 && value.trim() !== "";
...
const Checkout = (props) => {
  ...
  const confirmHandler = (event) => {
    ...
    const enteredPostalIsValid = isNotLongEnough(enteredPostal);
  ...
  /* 
  const inputControlClasses = (inputName) =>
    `${classes.control} ${formInputsValidity.inputName ? "" : classes.invalid}`;
 */
